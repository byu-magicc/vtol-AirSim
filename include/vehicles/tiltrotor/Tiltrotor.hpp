...
#ifndef msr_airlib_tiltrotor_hpp
#define msr_airlib_tiltrotor_hpp
...
class Tiltrotor : public PhysicsBody {
public:
    Tiltrotor(TiltrotorParams* params, VehicleApiBase* vehicle_api,
...
private: //methods
    ...
    static void createRotors(const TiltrotorParams& params, vector<RotorTiltrotor>& rotors, const Environment* environment)
    {
        ...
        for (uint rotor_index = 0; rotor_index < params.getParams().rotor_poses.size(); ++rotor_index) {
            const TiltrotorParams::RotorPose& rotor_pose = params.getParams().rotor_poses.at(rotor_index);
    ...
    void reportSensors(TiltrotorParams& params, StateReporter& reporter)
    {
        params.getSensors().reportState(reporter);
    }

    void updateSensors(TiltrotorParams& params, const Kinematics::State& state, const Environment& environment)
    {
        unused(state);
        unused(environment);
    ...
    void initSensors(TiltrotorParams& params, const Kinematics::State& state, const Environment& environment)
    {
