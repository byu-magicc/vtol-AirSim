...
#ifndef msr_airlib_TiltrotorSimpleCommon_hpp
...
class TiltrotorSimpleCommon {
public:
    static tiltrotor_simple::Axis3r toAxis3r(const Vector3r& vec)
    {
    ...
    static Vector3r toVector3r(const tiltrotor_simple::Axis3r& vec)
    {
        Vector3r conv;
        conv.x() = vec.x(); conv.y() = vec.y(); conv.z() = vec.z();
        return conv;
    ...
    static Kinematics::State toKinematicsState3r(const tiltrotor_simple::KinematicsState& state)
    {
    ...
    static tiltrotor_simple::Axis4r toAxis4r(const Quaternionr& q)
    {
        tiltrotor_simple::Axis4r conv;
    ...
    static Quaternionr toQuaternion(const tiltrotor_simple::Axis4r& q)
    {
        Quaternionr conv;
        conv.x() = q.x(); conv.y() = q.y(); conv.z() = q.z();
        conv.w() = q.val4();
    ...
    static tiltrotor_simple::GeoPoint toSimpleFlightGeoPoint(const GeoPoint& geo_point)
    {
        tiltrotor_simple::GeoPoint conv;
    ...
    static GeoPoint toGeoPoint(const tiltrotor_simple::GeoPoint& geo_point)
    {
        GeoPoint conv;
        conv.latitude = geo_point.latitude;
        conv.longitude = geo_point.longitude;
