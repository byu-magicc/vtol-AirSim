...
public:
    ...
    TiltrotorRpcLibClient* moveOnPathAsync(const vector<Vector3r>& path, float velocity, float timeout_sec = Utils::max<float>(),
        VTOLDrivetrainType drivetrain = VTOLDrivetrainType::MaxDegreeOfFreedom, const VTOLYawMode& yaw_mode = VTOLYawMode(),
    ...
    TiltrotorRpcLibClient* moveToZAsync(float z, float velocity, float timeout_sec = Utils::max<float>(),
        const VTOLYawMode& yaw_mode = VTOLYawMode(), float lookahead = -1, float adaptive_lookahead = 1, const std::string& vehicle_name = "");
