...
#include "vehicles/vtol/firmwares/mavlink/MavLinkOutputMappings.hpp"
...
namespace airlib
{
    ...
    class MavLinkVtolApi : public VtolApiBase
    {
    public: //methods
        virtual ~MavLinkVtolApi()
        {
            closeAllConnection();
        ...
        virtual void update() override
        {
            try {
                ...
                const auto& airspeed_output = getAirspeedData("");
        ...
        virtual void sendTelemetry(float last_interval = -1) override
        {
            ...
            // This method is called at high frequency from PawnSimApi::updateRendering.
        ...
        void start_telemtry_thread()
        {
            ...
            this->telemetry_thread_ = std::thread(&MavLinkVtolApi::telemtry_thread, this);
        }
    ...
    protected: //methods
        virtual void connect()
        {
            if (!connecting_) {
                ...
                this->connect_thread_ = std::thread(&MavLinkVtolApi::connect_thread, this);
            }
        }
    ...
    private: //methods
        ...
        void processMavMessages(const mavlinkcom::MavLinkMessage& msg)
        {
            if (msg.msgid == HeartbeatMessage.msgid) {
                ...
                throw std::invalid_argument("HilControlsMessage not supported in MavLinkVtolApi");
            }
    ...
    private: //variables
        ...
        std::shared_ptr<VtolApiBase> mav_vehicle_control_;
        ...
        //additional variables required for VtolApiBase implementation
