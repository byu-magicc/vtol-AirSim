...
namespace airlib
{
    ...
    VtolRpcLibServer::VtolRpcLibServer(ApiProvider* api_provider, string server_address, uint16_t port)
        : RpcLibServerBase(api_provider, server_address, port)
    {
        ...
        (static_cast<rpc::server*>(getServer()))->bind("moveOnPath", [&](const vector<VtolRpcLibAdaptors::Vector3r>& path, float velocity, float timeout_sec, DrivetrainType drivetrain, const MultirotorRpcLibAdaptors::YawMode& yaw_mode, float lookahead, float adaptive_lookahead, const std::string& vehicle_name) -> bool {
        ...
        (static_cast<rpc::server*>(getServer()))->bind("moveByRC", [&](const VtolRpcLibAdaptors::RCData& data, const std::string& vehicle_name) -> void {
            getVehicleApi(vehicle_name)->moveByRC(data.to());
        });

        (static_cast<rpc::server*>(getServer()))->bind("setSafety", [&](uint enable_reasons, float obs_clearance, const SafetyEval::ObsAvoidanceStrategy& obs_startegy, float obs_avoidance_vel, const VtolRpcLibAdaptors::Vector3r& origin, float xy_length, float max_z, float min_z, const std::string& vehicle_name) -> bool {
        ...
        (static_cast<rpc::server*>(getServer()))->bind("simSetVtolPose", [&](const VtolRpcLibAdaptors::Pose& pose, const vector<float>& tilt_angles, bool ignore_collision, bool spin_props, const std::string& vehicle_name) -> void {
            getVehicleSimApi(vehicle_name)->setPoseCustom(pose.to(), tilt_angles, ignore_collision, spin_props);
        });

        //getters
        // Rotor state
        (static_cast<rpc::server*>(getServer()))->bind("getRotorStates", [&](const std::string& vehicle_name) -> VtolRpcLibAdaptors::RotorTiltableStates {
            return VtolRpcLibAdaptors::RotorTiltableStates(getVehicleApi(vehicle_name)->getRotorStates());
        });
        // Vtol state
        (static_cast<rpc::server*>(getServer()))->bind("getVtolState", [&](const std::string& vehicle_name) -> VtolRpcLibAdaptors::VtolState {
            return VtolRpcLibAdaptors::VtolState(getVehicleApi(vehicle_name)->getVtolState());
        });
    }

    //required for pimpl
    VtolRpcLibServer::~VtolRpcLibServer()
    {
    }
}
} //namespace
