...
namespace airlib
{
    ...
    void VtolApiBase::resetImplementation()
    {
        cancelLastTask();
        SingleTaskCall lock(this); //cancel previous tasks
    }

    bool VtolApiBase::takeoff(float timeout_sec)
    {
    ...
    bool VtolApiBase::land(float timeout_sec)
    {
        SingleTaskCall lock(this);
    ...
    bool VtolApiBase::goHome(float timeout_sec)
    {
        SingleTaskCall lock(this);
    ...
    bool VtolApiBase::moveByVelocityBodyFrame(float vx, float vy, float vz, float duration, DrivetrainType drivetrain, const YawMode& yaw_mode)
    {
        SingleTaskCall lock(this);
    ...
    bool VtolApiBase::moveByVelocityZBodyFrame(float vx, float vy, float z, float duration, DrivetrainType drivetrain, const YawMode& yaw_mode)
    {
        SingleTaskCall lock(this);
    ...
    bool VtolApiBase::moveByMotorPWMs(const vector<float>& pwm_values, float duration)
    {
        SingleTaskCall lock(this);
    ...
    bool VtolApiBase::moveByRollPitchYawZ(float roll, float pitch, float yaw, float z, float duration)
    {
        SingleTaskCall lock(this);
    ...
    bool VtolApiBase::moveByRollPitchYawThrottle(float roll, float pitch, float yaw, float throttle, float duration)
    {
        SingleTaskCall lock(this);
    ...
    bool VtolApiBase::moveByRollPitchYawrateThrottle(float roll, float pitch, float yaw_rate, float throttle, float duration)
    {
        SingleTaskCall lock(this);
    ...
    bool VtolApiBase::moveByRollPitchYawrateZ(float roll, float pitch, float yaw_rate, float z, float duration)
    {
        SingleTaskCall lock(this);
    ...
    bool VtolApiBase::moveByAngleRatesZ(float roll_rate, float pitch_rate, float yaw_rate, float z, float duration)
    {
        SingleTaskCall lock(this);
    ...
    bool VtolApiBase::moveByAngleRatesThrottle(float roll_rate, float pitch_rate, float yaw_rate, float throttle, float duration)
    {
        SingleTaskCall lock(this);
    ...
    bool VtolApiBase::moveByVelocity(float vx, float vy, float vz, float duration, DrivetrainType drivetrain, const YawMode& yaw_mode)
    {
        SingleTaskCall lock(this);
    ...
    bool VtolApiBase::moveByVelocityZ(float vx, float vy, float z, float duration, DrivetrainType drivetrain, const YawMode& yaw_mode)
    {
        SingleTaskCall lock(this);
    ...
    bool VtolApiBase::moveOnPath(const vector<Vector3r>& path, float velocity, float timeout_sec, DrivetrainType drivetrain, const YawMode& yaw_mode,
                                      float lookahead, float adaptive_lookahead)
    {
        ...
        if (velocity > getVtolApiParams().breaking_vel) {
            breaking_dist = Utils::clip(velocity * getVtolApiParams().vel_to_breaking_dist,
                                        getVtolApiParams().min_breaking_dist,
                                        getVtolApiParams().max_breaking_dist);
        }
        ...
        while (!waiter.isTimeout() && (next_path_loc.seg_index < path_segs.size() - 1 || goal_dist > 0)) { //current position is approximately at the last end point
            ...
            if (seg_velocity > getVtolApiParams().min_vel_for_breaking && path_length_remaining <= breaking_dist) {
                seg_velocity = getVtolApiParams().breaking_vel;
    ...
    bool VtolApiBase::moveToPosition(float x, float y, float z, float velocity, float timeout_sec, DrivetrainType drivetrain,
    ...
    bool VtolApiBase::moveToZ(float z, float velocity, float timeout_sec, const YawMode& yaw_mode,
                                   float lookahead, float adaptive_lookahead)
    {
        SingleTaskCall lock(this);
    ...
    bool VtolApiBase::moveByManual(float vx_max, float vy_max, float z_min, float duration, DrivetrainType drivetrain, const YawMode& yaw_mode)
    {
        SingleTaskCall lock(this);
        ...
        do {
            ...
            if (rc_data.is_valid && (rc_data.timestamp == 0 || age <= kMaxMessageAge)) { //if rc message timestamp is not set OR is not too old
                ...
                catch (const VtolApiBase::UnsafeMoveException& ex) {
    ...
    bool VtolApiBase::rotateToYaw(float yaw, float timeout_sec, float margin)
    {
        SingleTaskCall lock(this);
    ...
    bool VtolApiBase::rotateByYawRate(float yaw_rate, float duration)
    {
        SingleTaskCall lock(this);
    ...
    void VtolApiBase::setAngleLevelControllerGains(const vector<float>& kp, const vector<float>& ki, const vector<float>& kd)
    {
        uint8_t controller_type = 2;
        setControllerGains(controller_type, kp, ki, kd);
    }

    void VtolApiBase::setAngleRateControllerGains(const vector<float>& kp, const vector<float>& ki, const vector<float>& kd)
    {
        uint8_t controller_type = 3;
        setControllerGains(controller_type, kp, ki, kd);
    }

    void VtolApiBase::setVelocityControllerGains(const vector<float>& kp, const vector<float>& ki, const vector<float>& kd)
    {
        uint8_t controller_type = 4;
        setControllerGains(controller_type, kp, ki, kd);
    }

    void VtolApiBase::setPositionControllerGains(const vector<float>& kp, const vector<float>& ki, const vector<float>& kd)
    {
        uint8_t controller_type = 5;
        setControllerGains(controller_type, kp, ki, kd);
    }

    bool VtolApiBase::hover()
    {
        SingleTaskCall lock(this);
    ...
    void VtolApiBase::moveByRC(const RCData& rc_data)
    {
        unused(rc_data);
        //by default we say that this command is not supported
        throw VehicleCommandNotImplementedException("moveByRC API is not implemented for this vtol");
    }

    void VtolApiBase::moveByVelocityInternal(float vx, float vy, float vz, const YawMode& yaw_mode)
    {
        if (safetyCheckVelocity(Vector3r(vx, vy, vz)))
            commandVelocity(vx, vy, vz, yaw_mode);
    }

    void VtolApiBase::moveByVelocityZInternal(float vx, float vy, float z, const YawMode& yaw_mode)
    {
        if (safetyCheckVelocityZ(vx, vy, z))
            commandVelocityZ(vx, vy, z, yaw_mode);
    }

    void VtolApiBase::moveToPositionInternal(const Vector3r& dest, const YawMode& yaw_mode)
    {
        if (safetyCheckDestination(dest))
            commandPosition(dest.x(), dest.y(), dest.z(), yaw_mode);
    }

    void VtolApiBase::moveByRollPitchYawZInternal(float roll, float pitch, float yaw, float z)
    {
        if (safetyCheckVelocity(getVelocity()))
            commandRollPitchYawZ(roll, pitch, yaw, z);
    }

    void VtolApiBase::moveByRollPitchYawThrottleInternal(float roll, float pitch, float yaw, float throttle)
    {
        if (safetyCheckVelocity(getVelocity()))
            commandRollPitchYawThrottle(roll, pitch, yaw, throttle);
    }

    void VtolApiBase::moveByRollPitchYawrateThrottleInternal(float roll, float pitch, float yaw_rate, float throttle)
    {
        if (safetyCheckVelocity(getVelocity()))
            commandRollPitchYawrateThrottle(roll, pitch, yaw_rate, throttle);
    }

    void VtolApiBase::moveByRollPitchYawrateZInternal(float roll, float pitch, float yaw_rate, float z)
    {
        if (safetyCheckVelocity(getVelocity()))
            commandRollPitchYawrateZ(roll, pitch, yaw_rate, z);
    }

    void VtolApiBase::moveByAngleRatesZInternal(float roll_rate, float pitch_rate, float yaw_rate, float z)
    {
        if (safetyCheckVelocity(getVelocity()))
            commandAngleRatesZ(roll_rate, pitch_rate, yaw_rate, z);
    }

    void VtolApiBase::moveByAngleRatesThrottleInternal(float roll_rate, float pitch_rate, float yaw_rate, float throttle)
    {
        if (safetyCheckVelocity(getVelocity()))
    ...
    Waiter VtolApiBase::waitForFunction(WaitFunction function, float timeout_sec)
    {
        Waiter waiter(getCommandPeriod(), timeout_sec, getCancelToken());
        if (timeout_sec <= 0)
            return waiter;
    ...
    bool VtolApiBase::waitForZ(float timeout_sec, float z, float margin)
    {
        float cur_z = 100000;
        return waitForFunction([&]() {
                   cur_z = getPosition().z();
    ...
    void VtolApiBase::setSafetyEval(const shared_ptr<SafetyEval> safety_eval_ptr)
    {
        SingleCall lock(this);
        safety_eval_ptr_ = safety_eval_ptr;
    }

    RCData VtolApiBase::estimateRCTrims(float trimduration, float minCountForTrim, float maxTrim)
    {
    ...
    void VtolApiBase::moveToPathPosition(const Vector3r& dest, float velocity, DrivetrainType drivetrain, /* pass by value */ YawMode yaw_mode, float last_z)
    {
        unused(last_z);
        //validate dest
        if (dest.hasNaN())
    ...
    bool VtolApiBase::setSafety(SafetyEval::SafetyViolationType enable_reasons, float obs_clearance, SafetyEval::ObsAvoidanceStrategy obs_startegy,
    ...
    bool VtolApiBase::emergencyManeuverIfUnsafe(const SafetyEval::EvalResult& result)
    {
        if (!result.is_safe) {
    ...
    bool VtolApiBase::safetyCheckVelocity(const Vector3r& velocity)
    {
    ...
    bool VtolApiBase::safetyCheckVelocityZ(float vx, float vy, float z)
    {
        if (safety_eval_ptr_ == nullptr) //safety checks disabled
            return true;
    ...
    bool VtolApiBase::safetyCheckDestination(const Vector3r& dest_pos)
    {
        if (safety_eval_ptr_ == nullptr) //safety checks disabled
            return true;
    ...
    float VtolApiBase::setNextPathPosition(const vector<Vector3r>& path, const vector<PathSegment>& path_segs,
    ...
    void VtolApiBase::adjustYaw(const Vector3r& heading, DrivetrainType drivetrain, YawMode& yaw_mode)
    {
    ...
    void VtolApiBase::adjustYaw(float x, float y, DrivetrainType drivetrain, YawMode& yaw_mode)
    {
        adjustYaw(Vector3r(x, y, 0), drivetrain, yaw_mode);
    }

    bool VtolApiBase::isYawWithinMargin(float yaw_target, float margin) const
    {
    ...
    float VtolApiBase::getObsAvoidanceVelocity(float risk_dist, float max_obs_avoidance_vel) const
    {
        unused(risk_dist);
