...
#include "SimModeWorldTiltrotor.h"
...
void ASimModeWorldTiltrotor::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
...
void ASimModeWorldTiltrotor::setupClockSpeed()
{
...
std::unique_ptr<msr::airlib::ApiServerBase> ASimModeWorldTiltrotor::createApiServer() const
{
#ifdef AIRLIB_NO_RPC
...
void ASimModeWorldTiltrotor::getExistingVehiclePawns(TArray<AActor*>& pawns) const
{
...
PawnEvents* ASimModeWorldTiltrotor::getVehiclePawnEvents(APawn* pawn) const
{
    return static_cast<TVehiclePawn*>(pawn)->getPawnEvents();
}
const common_utils::UniqueValueMap<std::string, APIPCamera*> ASimModeWorldTiltrotor::getVehiclePawnCameras(
    APawn* pawn) const
{
    return (static_cast<const TVehiclePawn*>(pawn))->getCameras();
}
void ASimModeWorldTiltrotor::initializeVehiclePawn(APawn* pawn)
{
    static_cast<TVehiclePawn*>(pawn)->initializeForBeginPlay();
}
std::unique_ptr<PawnSimApi> ASimModeWorldTiltrotor::createVehicleSimApi(
    const PawnSimApi::Params& pawn_sim_api_params) const
{
    auto vehicle_sim_api = std::unique_ptr<PawnSimApi>(new TiltrotorPawnSimApi(pawn_sim_api_params));
