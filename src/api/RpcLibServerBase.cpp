    ...
    : api_provider_(api_provider)
{
    ...
    pimpl_->server.bind("getAirspeedData", [&](const std::string& airspeed_name, const std::string& vehicle_name) -> RpcLibAdapatorsBase::AirspeedData {
        const auto& airspeed_data = getVehicleApi(vehicle_name)->getAirspeedData(airspeed_name);
        return RpcLibAdapatorsBase::AirspeedData(airspeed_data);
    });
