...
namespace airlib
{
        ...
        : RpcLibServerBase(api_provider, server_address, port)
    {
        ...
        (static_cast<rpc::server*>(getServer()))->bind("moveByPWMs", [&](const vector<float>& pwm_values, float duration, const std::string& vehicle_name) -> bool {
            return getVehicleApi(vehicle_name)->moveByPWMs(pwm_values, duration);
        });
        ...
        (static_cast<rpc::server*>(getServer()))->bind("moveOnPath", [&](const vector<TiltrotorRpcLibAdaptors::Vector3r>& path, float velocity, float timeout_sec, DrivetrainType drivetrain, const MultirotorRpcLibAdaptors::YawMode& yaw_mode, float lookahead, float adaptive_lookahead, const std::string& vehicle_name) -> bool {
        ...
        (static_cast<rpc::server*>(getServer()))->bind("moveByRC", [&](const TiltrotorRpcLibAdaptors::RCData& data, const std::string& vehicle_name) -> void {
            getVehicleApi(vehicle_name)->moveByRC(data.to());
        });

        (static_cast<rpc::server*>(getServer()))->bind("setSafety", [&](uint enable_reasons, float obs_clearance, const SafetyEval::ObsAvoidanceStrategy& obs_startegy, float obs_avoidance_vel, const TiltrotorRpcLibAdaptors::Vector3r& origin, float xy_length, float max_z, float min_z, const std::string& vehicle_name) -> bool {
        ...
        (static_cast<rpc::server*>(getServer()))->bind("simSetTiltrotorPose", [&](const TiltrotorRpcLibAdaptors::Pose& pose, const vector<float>& tilt_angles, bool ignore_collision, bool spin_props, const std::string& vehicle_name) -> void {
            getVehicleSimApi(vehicle_name)->setPoseCustom(pose.to(), tilt_angles, ignore_collision, spin_props);
        });

        //getters
        // Rotor state
        (static_cast<rpc::server*>(getServer()))->bind("getRotorStates", [&](const std::string& vehicle_name) -> TiltrotorRpcLibAdaptors::RotorTiltableStates {
            return TiltrotorRpcLibAdaptors::RotorTiltableStates(getVehicleApi(vehicle_name)->getRotorStates());
        });
        // Tiltrotor state
        (static_cast<rpc::server*>(getServer()))->bind("getTiltrotorState", [&](const std::string& vehicle_name) -> TiltrotorRpcLibAdaptors::TiltrotorState {
