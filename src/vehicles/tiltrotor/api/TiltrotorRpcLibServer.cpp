        ...
        : RpcLibServerBase(api_provider, server_address, port)
{
    ...
    (static_cast<rpc::server*>(getServer()))->
        bind("moveByRC", [&](const TiltrotorRpcLibAdaptors::RCData& data, const std::string& vehicle_name) -> void {
    ...
    (static_cast<rpc::server*>(getServer()))->
        ...
        float obs_avoidance_vel, const TiltrotorRpcLibAdaptors::Vector3r& origin, float xy_length,
