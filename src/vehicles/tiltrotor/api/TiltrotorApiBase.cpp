...
void TiltrotorApiBase::resetImplementation()
{
    cancelLastTask();
    SingleTaskCall lock(this); //cancel previous tasks
}

bool TiltrotorApiBase::takeoff(float timeout_sec)
{
...
bool TiltrotorApiBase::land(float timeout_sec)
{
    SingleTaskCall lock(this);
...
bool TiltrotorApiBase::goHome(float timeout_sec)
{
    SingleTaskCall lock(this);
...
bool TiltrotorApiBase::moveByRollPitchYawThrottle(float roll, float pitch, float yaw, float throttle, float duration)
{
    SingleTaskCall lock(this);
...
bool TiltrotorApiBase::moveByRollPitchYawrateThrottle(float roll, float pitch, float yaw_rate, float throttle, float duration)
{
    SingleTaskCall lock(this);
...
bool TiltrotorApiBase::moveByRollPitchYawrateZ(float roll, float pitch, float yaw_rate, float z, float duration)
{
    SingleTaskCall lock(this);
...
bool TiltrotorApiBase::moveByAngleRatesZ(float roll_rate, float pitch_rate, float yaw_rate, float z, float duration)
{
    SingleTaskCall lock(this);
...
bool TiltrotorApiBase::moveByAngleRatesThrottle(float roll_rate, float pitch_rate, float yaw_rate, float throttle, float duration)
{
    SingleTaskCall lock(this);
    ...
    float lookahead, float adaptive_lookahead)
{
    ...
    if (velocity > getTiltrotorApiParams().breaking_vel) {
        breaking_dist = Utils::clip(velocity * getTiltrotorApiParams().vel_to_breaking_dist,
            getTiltrotorApiParams().min_breaking_dist, getTiltrotorApiParams().max_breaking_dist);
    }
    ...
    while (!waiter.isTimeout() && (next_path_loc.seg_index < path_segs.size()-1 || goal_dist > 0)
        ...
        if (seg_velocity > getTiltrotorApiParams().min_vel_for_breaking && path_length_remaining <= breaking_dist) {
            seg_velocity = getTiltrotorApiParams().breaking_vel;
...
bool TiltrotorApiBase::moveByManual(float vx_max, float vy_max, float z_min, float duration, DrivetrainType drivetrain, const YawMode& yaw_mode)
{
    ...
    do {
        ...
        if (rc_data.is_valid && (rc_data.timestamp == 0 || age <= kMaxMessageAge)) { //if rc message timestamp is not set OR is not too old
            ...
            catch (const TiltrotorApiBase::UnsafeMoveException& ex) {
...
bool TiltrotorApiBase::rotateToYaw(float yaw, float timeout_sec, float margin)
{
    SingleTaskCall lock(this);
...
void TiltrotorApiBase::setAngleRateControllerGains(const vector<float>& kp, const vector<float>& ki, const vector<float>& kd)
{
    uint8_t controller_type = 3;
    setControllerGains(controller_type, kp, ki, kd);
}

void TiltrotorApiBase::setVelocityControllerGains(const vector<float>& kp, const vector<float>& ki, const vector<float>& kd)
{
    uint8_t controller_type = 4;
    setControllerGains(controller_type, kp, ki, kd);
}

void TiltrotorApiBase::setPositionControllerGains(const vector<float>& kp, const vector<float>& ki, const vector<float>& kd)
{
    uint8_t controller_type = 5;
    setControllerGains(controller_type, kp, ki, kd);
}

bool TiltrotorApiBase::hover()
{
    SingleTaskCall lock(this);
...
void TiltrotorApiBase::moveByRollPitchYawZInternal(float roll, float pitch, float yaw, float z)
{
    if (safetyCheckVelocity(getVelocity()))
        commandRollPitchYawZ(roll, pitch, yaw, z);
}

void TiltrotorApiBase::moveByRollPitchYawThrottleInternal(float roll, float pitch, float yaw, float throttle)
{
    if (safetyCheckVelocity(getVelocity()))
        commandRollPitchYawThrottle(roll, pitch, yaw, throttle);
}

void TiltrotorApiBase::moveByRollPitchYawrateThrottleInternal(float roll, float pitch, float yaw_rate, float throttle)
{
    if (safetyCheckVelocity(getVelocity()))
        commandRollPitchYawrateThrottle(roll, pitch, yaw_rate, throttle);
}

void TiltrotorApiBase::moveByRollPitchYawrateZInternal(float roll, float pitch, float yaw_rate, float z)
{
    if (safetyCheckVelocity(getVelocity()))
        commandRollPitchYawrateZ(roll, pitch, yaw_rate, z);
}

void TiltrotorApiBase::moveByAngleRatesZInternal(float roll_rate, float pitch_rate, float yaw_rate, float z)
{
    if (safetyCheckVelocity(getVelocity()))
        commandAngleRatesZ(roll_rate, pitch_rate, yaw_rate, z);
}

void TiltrotorApiBase::moveByAngleRatesThrottleInternal(float roll_rate, float pitch_rate, float yaw_rate, float throttle)
{
    if (safetyCheckVelocity(getVelocity()))
...
Waiter TiltrotorApiBase::waitForFunction(WaitFunction function, float timeout_sec)
{
    Waiter waiter(getCommandPeriod(), timeout_sec, getCancelToken());
    if (timeout_sec <= 0)
        return waiter;
...
bool TiltrotorApiBase::waitForZ(float timeout_sec, float z, float margin)
{
    float cur_z = 100000;
    return waitForFunction([&]() {
        cur_z = getPosition().z();
...
void TiltrotorApiBase::setSafetyEval(const shared_ptr<SafetyEval> safety_eval_ptr)
{
    SingleCall lock(this);
    safety_eval_ptr_ = safety_eval_ptr;
}

RCData TiltrotorApiBase::estimateRCTrims(float trimduration, float minCountForTrim, float maxTrim)
{
...
bool TiltrotorApiBase::setSafety(SafetyEval::SafetyViolationType enable_reasons, float obs_clearance, SafetyEval::ObsAvoidanceStrategy obs_startegy,
...
bool TiltrotorApiBase::emergencyManeuverIfUnsafe(const SafetyEval::EvalResult& result)
{
    if (!result.is_safe) {
...
bool TiltrotorApiBase::safetyCheckVelocity(const Vector3r& velocity)
{
...
bool TiltrotorApiBase::safetyCheckVelocityZ(float vx, float vy, float z)
{
    if (safety_eval_ptr_ == nullptr) //safety checks disabled
        return true;
...
bool TiltrotorApiBase::safetyCheckDestination(const Vector3r& dest_pos)
{
    if (safety_eval_ptr_ == nullptr) //safety checks disabled
        return true;
...
float TiltrotorApiBase::setNextPathPosition(const vector<Vector3r>& path, const vector<PathSegment>& path_segs,
...
bool TiltrotorApiBase::isYawWithinMargin(float yaw_target, float margin) const
{
    const float yaw_current = VectorMath::getYaw(getOrientation()) * 180 / M_PIf;
    return std::abs(yaw_current - yaw_target) <= margin;
}

float TiltrotorApiBase::getAutoLookahead(float velocity, float adaptive_lookahead,
...
float TiltrotorApiBase::getObsAvoidanceVelocity(float risk_dist, float max_obs_avoidance_vel) const
{
    unused(risk_dist);
