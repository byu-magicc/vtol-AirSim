    ...
    namespace msr
{
    namespace airlib
    {
        ...
        TiltrotorRpcLibClient* TiltrotorRpcLibClient::moveToZAsync(float z, float velocity, float timeout_sec, const YawMode& yaw_mode, float lookahead, float adaptive_lookahead, const std::string& vehicle_name)
        {
        ...
        {
            return static_cast<rpc::client*>(getClient())->call("setSafety", static_cast<uint>(enable_reasons), obs_clearance, obs_startegy, obs_avoidance_vel, TiltrotorRpcLibAdaptors::Vector3r(origin), xy_length, max_z, min_z, vehicle_name).as<bool>();
        }
        ...
        {
            return static_cast<rpc::client*>(getClient())->call("getRotorStates", vehicle_name).as<TiltrotorRpcLibAdaptors::RotorTiltableStates>().to();
        }
        // Tiltrotor state getter
        TiltrotorState TiltrotorRpcLibClient::getTiltrotorState(const std::string& vehicle_name)
        {
            return static_cast<rpc::client*>(getClient())->call("getTiltrotorState", vehicle_name).as<TiltrotorRpcLibAdaptors::TiltrotorState>().to();
        }
